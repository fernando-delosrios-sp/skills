...
import { assertSafeGitRef, runGit } from './git-exec.mjs';
...
function gitShow(ref, path) {
  ...
  try {
    return runGit(['show', `${ref}:${path}`], { cwd: ROOT });
  } catch {
    return null;
  }
}
...
function gitLsTree(ref, prefix) {
  assertSafeGitRef(ref);
  try {
    return runGit(['ls-tree', '-r', ref, '--name-only', prefix], { cwd: ROOT })
      .trim()
      .split('\n')
      .filter(Boolean);
  } catch {
  ...
  for (const change of diff) {
    if (change.type === 'local_only') {
      ...
      if (skill && (await isGeneratedPathForSkill(skillName, change.file))) {
        const skillDir = skillDirOverride ?? getCanonicalDir(skill);
...
async function writeOverlayFromChanges(
  ...
  for (const change of overlayChanges) {
    if (change._content !== undefined) {
      const dest = resolveContained(filesDir, change.file);
...
async function resolveLocalFiles(skill, { fromAgents = false, fromCommit = null } = {}) {
  ...
  const localDir = fromAgents ? getAgentsDir(skill) : getCanonicalDir(skill);
