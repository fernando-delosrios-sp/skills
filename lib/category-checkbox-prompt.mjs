...
function switchCategory(categories, activeCategory, delta, setActiveCategory, setActive) {
  const nextCategory = (activeCategory + delta + categories.length) % categories.length;
  setActiveCategory(nextCategory);
  setActive(firstSelectableIndex(categories[nextCategory].items));
}
...
export const categoryCheckbox = createPrompt((config, done) => {
  const { pageSize = 10, loop = false, required = false, validate = () => true } = config;
  ...
  const [active, setActive] = useState(() => firstSelectableIndex(categories[0]?.items ?? []));
  ...
  useKeypress(async (key) => {
    if (isEnterKey(key)) {
      ...
      switchCategory(categories, activeCategory, -1, setActiveCategory, setActive);
      ...
      while (next >= bounds.first && next <= bounds.last) {
        if (isSelectable(currentItems[next])) {
          setActive(next);
          break;
        }
        next += offset;
  ...
  let description;
  const listActive = active < currentItems.length ? active : bounds.first;
