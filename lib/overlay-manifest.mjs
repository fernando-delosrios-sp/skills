...
- [ ] Record blend in \`.locks/upstream.json\`: applied_upstream_sha, overlay_hash, universal_overlay_hash, overlay_applied_at, blended_ref (git rev-parse HEAD after the tree commit)
- [ ] Commit \`.locks/upstream.json\` after recordBlend so overlay_applied_at is set
- [ ] If pushing: push after the lock commit so remotes include overlay_applied_at`;
...
export async function prepareOverlayManifest(skillName, { runStatic = true } = {}) {
  ...
  const lines = [
    ...
    `**Skill dir:** ${getGitSkillPrefix(skill)}/`,
    ...
    '',
    'Apply semantic changes using the **update-skills** skill (apply-only mode).',
    '',
...
export async function prepareGeneratorManifest(skillName) {
  ...
  const lines = [
    ...
    `**Synced at:** ${lock?.synced_at ?? 'unknown'}`,
    '',
    'Apply generators using the **update-skills** skill (apply-only mode).',
    '',
    '## Semantic changes',
    '',
