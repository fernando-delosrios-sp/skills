...
// Do not import from overlay-pipeline.mjs here:
// overlay-pipeline -> overlay-extract -> tmp would create a cycle.
// overlay-audit owns the implementation that the pipeline re-exports.
import { isPendingApply } from './overlay-audit.mjs';
...
export async function cleanOverlayManifests({ appliedOnly = true } = {}) {
  ...
  for (const entry of entries) {
    ...
    if (appliedOnly && (await isPendingApply(skillName))) continue;
