...
import {
  getLockEntry,
  isOverlayRoutePending,
  getOverlayRoute,
  recordBlend,
} from './locks.mjs';
import { resolveGeneratorsForSkill, GLOBAL_OVERLAY_PATH } from './generator-config.mjs';
...
export async function hashUniversalOverlay() {
  const content = await readFile(GLOBAL_OVERLAY_PATH, 'utf8');
  return createHash('sha256').update(content).digest('hex');
}

function getGitSkillPrefix(skill) {
...
export async function getCurrentOverlayHashes(skillName) {
...
export async function auditSkill(skillName) {
  ...
  const hashes = await getCurrentOverlayHashes(skillName);
  ...
  const blendedRefValid = isBlendedRefValid(lock?.blended_ref, skill);
...
export async function auditAllSkills({ skillName = null } = {}) {
  if (skillName) {
    return [await auditSkill(skillName)];
  }
  ...
  for (const skill of skills) {
    ...
    if (hashes.hasPerSkillOverlay || hashes.hasGenerators) {
      results.push(await auditSkill(skill.name));
    }
  }
...
export async function restoreSkill(skillName, { dryRun = false } = {}) {
  ...
  if (!dryRun) {
    try {
      execSync(`git checkout ${audit.blended_ref} -- ${gitPrefix}`, {
        cwd: ROOT,
        stdio: 'pipe',
      });
    } catch (err) {
...
export async function restoreAllSkills({ skillName = null, dryRun = false } = {}) {
  const audits = await auditAllSkills({ skillName });
  const results = [];
  ...
  for (const audit of audits) {
    ...
    results.push(await restoreSkill(audit.skill, { dryRun }));
  }
...
export async function listPendingOverlaySkills() {
  const audits = await auditAllSkills();
  return audits.filter((a) => isOverlayRoutePending(a.route)).map((a) => a.skill);
}
...
export async function prepareOverlays({ skillName = null, runStatic = true } = {}) {
  ...
  const results = [];
  for (const name of targets) {
    if (await hasOverlay(name)) {
      results.push(await prepareOverlayManifest(name, { runStatic }));
    } else {
      results.push(await prepareGeneratorManifest(name));
    }
  }
  return results;
}
