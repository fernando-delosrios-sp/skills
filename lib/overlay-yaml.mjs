...
import { getCanonicalDir, getOverlayDir } from './skill-paths.mjs';
...
// A terminator must be followed by whitespace or end of text, so periods inside
// filenames and paths (tasks.md, .drawio) do not end the sentence.
...
export async function expectedContentForPath(skill, relPath, { skillDir } = {}) {
  ...
  const dir = skillDir ?? getCanonicalDir(skill);
  let skillMd;
  try {
