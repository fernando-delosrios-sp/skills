...
import { TMP_DIR, cleanRepoClone, repoCloneDirName } from './tmp.mjs';
import { resolveContained } from './skill-paths.mjs';
...
async function writeUpstreamToLocal(localSkillDir, upstreamFiles) {
  ...
  for (const f of upstreamFiles) {
    ...
    const localPath = resolveContained(localSkillDir, f.relPath);
...
/**
 * Read one skill from its upstream repo.
 *
 * Pass a `cloneCache` (Map of repoRef → clone dir) to reuse one clone across
 * every skill sourced from the same repo. The cache owner is then responsible
 * for cleanup; without a cache each call clones and cleans its own dir.
 */
export async function syncSkill(skill, locks, { cloneCache = null, cloneRepoFn = cloneRepo } = {}) {
  if (!skill.source) {
    return { skill: skill.name, status: 'skipped', reason: 'no source (local skill)' };
  }

  const { repo } = skill.source;
  const tmpDir = cloneCache?.get(repo) ?? resolve(TMP_DIR, repoCloneDirName(repo));
  const reusingClone = cloneCache?.has(repo) ?? false;
  ...
  try {
    if (!reusingClone) {
      ...
      try {
        await cloneRepoFn(repo, tmpDir);
      } catch (err) {
        return {
          skill: skill.name,
          status: 'error',
          reason: `Failed to clone ${repo}: ${err.message}`,
        };
      }

      cloneCache?.set(repo, tmpDir);
    }
...
export async function syncAllSkills({ dryRun = false } = {}) {
  ...
  try {
    for (const skill of skills) {
      ...
      const result = await syncSkill(skill, locks, { cloneCache });
    ...
    for (const repo of cloneCache.keys()) {
      await cleanRepoClone(repo);
    }
  }
