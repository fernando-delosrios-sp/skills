import { readFile, access, mkdir, readdir, stat, rm, writeFile } from 'node:fs/promises';
import { resolve, relative, dirname } from 'node:path';
import { execSync } from 'node:child_process';
import { loadSkills, getSkillDir } from './index.mjs';
import { loadLocks, saveLocks, isOverlayPending } from './locks.mjs';
import { hasOverlay } from './overlays.mjs';
...
function getUpstreamSha(cloneDir) {
...
async function writeUpstreamToLocal(upstreamSkillDir, localSkillDir, upstreamFiles) {
  const upstreamRelPaths = new Set();

  for (const f of upstreamFiles) {
    const relPath = relative(upstreamSkillDir, f.path);
    upstreamRelPaths.add(relPath);
    const localPath = resolve(localSkillDir, relPath);
...
export async function syncSkill(skill, locks) {
  ...
  try {
    ...
    const upstreamSha = getUpstreamSha(tmpDir);
    ...
    if (existingLock?.sha === upstreamSha) {
      const overlayExists = await hasOverlay(skill.name);
...
export async function applySyncResult(result, locks) {
  ...
  await writeUpstreamToLocal(
    result.upstreamSkillDir,
    result.localSkillDir,
    result.upstreamFiles
  );
...
export function printSyncSummary(results) {
  ...
  if (overlayPending.length > 0) {
    ...
    console.log(kleur.dim('Then run skill-overlay apply in Cursor for remerge skills.'));
  }
