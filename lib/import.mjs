...
import { prepareGeneratorManifest } from './overlays.mjs';
...
export async function importSkill(repoOwner, repoPath, category) {
  ...
  let cloneRoot;
  try {
    cloneRoot = await cloneRepo(repoOwner);
    ...
    try {
      const manifest = await prepareGeneratorManifest(result.name);
      console.log(kleur.dim(`  Generator manifest: ${manifest.manifestPath}`));
      console.log(kleur.dim('  In Cursor: apply generators for this skill via apply-skill-overlay'));
    } catch (err) {
      console.log(kleur.dim(`  Generators: ${err.message}`));
    }
    return result;
  } finally {
