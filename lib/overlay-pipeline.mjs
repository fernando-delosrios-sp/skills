...
export async function validateOverlays(skills) {
  ...
  for (const name of overlayNames) {
    ...
    for (const change of overlay.changes) {
      try {
        resolveContained(getCanonicalDir(skill), change.file);
      } catch (err) {
        errors.push({
          type: 'overlay',
          message: `Overlay "${name}": invalid target ${change.file}: ${err.message}`,
        });
      }
    }
    ...
    for (const op of staticOps) {
      if ((op.action === 'add' || op.action === 'replace') && op.from) {
        try {
          const sourcePath = resolveContained(overlay._dir, op.from);
          await access(sourcePath);
        } catch (err) {
          errors.push({
            type: 'overlay',
            message: err.message.includes('path escapes directory') || err.message.includes('absolute path')
              ? `Overlay "${name}": invalid static source ${op.from}: ${err.message}`
              : `Overlay "${name}": static source not found: ${op.from}`,
          });
        }
      }
