...
import { ROOT, getCanonicalDir } from './skill-paths.mjs';
...
const README_CATEGORY_ROW_RE = /^\|\s*\*\*([^*]+)\*\*\s*\|\s*([^|\n]*)\|/gm;

export function compareReadmeCatalog(readmeText, skills) {
  const expectedByCategory = new Map();
  for (const skill of skills) {
    if (!skill?.category || !skill?.name) continue;
    if (!expectedByCategory.has(skill.category)) {
      expectedByCategory.set(skill.category, new Set());
    }
    expectedByCategory.get(skill.category).add(skill.name);
  }

  const actualByCategory = new Map();
  for (const match of readmeText.matchAll(README_CATEGORY_ROW_RE)) {
    const category = match[1].trim();
    const names = match[2]
      .split(',')
      .map((name) => name.trim())
      .filter(Boolean);
    actualByCategory.set(category, new Set(names));
  }
  ...
  for (const [category, expected] of expectedByCategory) {
    ...
    if (!actual) {
      ...
      errors.push({
        type: 'readme-catalog',
        message: `README category catalog missing row for "${category}"; missing: ${missing.join(', ')}`,
      });
      continue;
    }
    ...
    if (missing.length > 0 || extra.length > 0) {
      const parts = [`README category catalog for "${category}" does not match manifests`];
      if (missing.length > 0) parts.push(`missing: ${missing.join(', ')}`);
      if (extra.length > 0) parts.push(`extra: ${extra.join(', ')}`);
      errors.push({ type: 'readme-catalog', message: parts.join('; ') });
    }
  }
...
export async function validateStructure() {
  ...
  try {
    ...
    try {
      ...
      errors.push(...compareReadmeCatalog(readmeText, skills));
    } catch {
      errors.push({
        type: 'readme-catalog',
        message: 'README category catalog missing: README.md could not be read',
      });
    }
    ...
    if (skill) {
      ...
      for (const gen of generators) {
        ...
        try {
          ...
          warnings.push({
            type: 'generated-file',
            message: `Missing ${gen.file} in ${name} (generator: ${gen.id}) — run npm run overlay -- prepare-generators and apply via update-skills`,
            skill: name,
          });
        }
      }
  ...
  for (const skill of skills) {
    ...
    try {
      ...
      if (isOverlayRoutePending(audit.route) && !audit.blended_ref) {
        warnings.push({
          type: 'overlay-lock',
          message: `Skill "${skill.name}" pending ${audit.route} with no blended_ref — apply via update-skills then commit`,
          skill: skill.name,
        });
      }
    } catch {
