...
- **apply-code-changes artifact checkout** — Introduce `ACTIVE_CHANGE_ROOT` so `tracking.md` and other artifacts persist on the feature branch/worktree where work runs; split subagent brief by workspace (`local` vs worktree paths).
