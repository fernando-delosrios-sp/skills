...
- **structured-choices drops the text fallback** — A gate is now a question-tool call and nothing else: `references/fallback.md` and the invented `<decision_prompt>` contract are deleted, since every leaked gate traced back to one of those two artifacts sitting in context as something concrete to emit. A host that does not answer the call falls under `Skip the gate` — pick a documented default and state the assumption, rather than asking in text.
- **structured-choices payloads match the Cursor schema** — Examples passed an `options[].detail` field that Cursor `AskQuestion` does not accept, so a copied payload could be rejected and drop the gate into its fallback. Detail now folds into `label`. `payload-examples.md` holds only question-tool call shapes, and the `<decision_prompt>` contract moved to `fallback.md` so it is unreachable from the tool-call path.
